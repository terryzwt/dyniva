...
namespace Drupal\dyniva_comment\Plugin\Field\FieldFormatter;
...
class CommentFormatter extends CommentDefaultFormatter {
  ...
  public function viewElements(FieldItemListInterface $items, $langcode) {
    ...
    if ($status != CommentWidget::CLOSED && empty($entity->in_preview) &&
      ...
      if ($this->currentUser->hasPermission('access comments') || $this->currentUser->hasPermission('administer comments')) {
        ...
        if ($status != CommentWidget::CLOSED && ($entity->get($field_name)->comment_count || $this->currentUser->hasPermission('administer comments'))) {
          $mode = $comment_settings['default_mode'];
          $comments_per_page = $comment_settings['per_page'];
          $comments = $this->storage->loadThread($entity, $field_name, $mode, $comments_per_page, $this->getSetting('pager_id'));
          
          $filters = $comments;
          foreach ($filters as $cid => $comment){
            if($comment->hasParentComment()){
              $p = $comment->getParentComment();
              if($p->status->value == 0){
                unset($comments[$cid]);
              }
            }
      ...
      if ($status != CommentWidget::CLOSED && $comment_settings['form_location'] == CommentItemInterface::FORM_BELOW && $this->viewMode != 'print') {
