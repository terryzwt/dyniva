...
class BulkUploadForm extends FormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state, $bundle = NULL) {
    ...
    if (empty($bundle)) {
      ...
      if (!empty($managedEntitys)) {
        foreach ($managedEntitys as $managedEntity) {
          $bundles[] = $managedEntity->getBundle();
        }
      }
    }
    else {
    ...
    $extensions = $this->helper->getFileExtensions(TRUE, $bundles);
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    if ($bulk_create) {
      ...
      $entity = array_shift($bulk_create);
      if(\Drupal::moduleHandler()->moduleExists('ccms_core')){
        $managed_entity = ccms_core_get_entity_managed_entity($entity);
        if ($managed_entity) {
          $redirect = Url::fromRoute("ccms_core.managed_entity.{$managed_entity->id()}.edit_page", ['managed_entity_id' => $entity->id()], [
            'query' => [
        ...
        if ($managed_entity) {
          $redirect = Url::fromRoute("dyniva_core.managed_entity.{$managed_entity->id()}.edit_page", ['managed_entity_id' => $entity->id()], [
            'query' => [
              'bulk-create' => $bulk_create,
            ],
          ]);
